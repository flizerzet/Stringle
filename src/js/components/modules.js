...
export function checkMobile() {
	if (isMobile.any()) { document.querySelector('html').classList.add('touch'); }
}
...
export function addFixedHeader() {
	...
	function fixHeader() {
		if (window.scrollY > header.clientHeight) {
			...
			page.style.marginTop = header.innerHeight;
		} else {
...
export class Select {
	...
	clickHandler(event) {
		...
		if (type === 'input' || type === 'value' || event.target.closest('.select__icon')) {
			this.toggle();
		} else if (type === 'item') {
